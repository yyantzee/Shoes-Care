...
include 'config.php';
$sql = "SELECT tb_transaction.id,tb_user.email,tb_order.id_order,tb_order.type_shoes,tb_order.quantity,tb_order.payment FROM tb_transaction INNER JOIN tb_user ON tb_user.id_user=tb_transaction.id_user INNER JOIN tb_order ON tb_order.id_order=tb_transaction.id_order ";
...
if (isset($_POST['delete'])) {
  ...
  header("location: tabletransaction.php");
}
...
if (isset($_POST['update'])) {
  ...
  $query = "UPDATE tb_order SET type_shoes = '$type_shoes', quantity = '$quantity' , payment = '$payment' WHERE id_order = $id";
...
if (isset($_GET['cari'])) {
  $cari = $_GET['cari'];
  $sql = " SELECT tb_transaction.id,tb_user.email,tb_order.type_shoes,tb_order.quantity,tb_order.payment FROM tb_transaction INNER JOIN tb_user ON tb_user.id_user=tb_transaction.id_user INNER JOIN tb_order ON tb_order.id_order=tb_transaction.id_order  WHERE id LIKE '%$cari%' OR email LIKE '%$cari%' OR type_shoes LIKE '%$cari%' OR quantity LIKE '%$cari%' OR payment LIKE '%$cari%'";
...
<h3 class="text-center font-extrabold text-4xl mb-10">TABEL TRANSAKSI</h3>
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  ...
                  <form action="" method="post">
                    <div class="p-6 text-center">
                      ...
                      <form action="" method="post">
                      <input type="text" name="id_deletetransaction" value="<?php echo $rows['id'] ?>" hidden>
                      <input type="text" name="id_deleteorder" value="<?php echo $rows['id_order'] ?>" hidden>
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="authentication-modal<?php echo $nomor?>">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                      ...
                      <input type="text" name="id_order" value="<?php echo $rows['id_order'] ?>" hidden>
                      <div>
                          <label for="type_shoes" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your Type Shoes</label>
                          <select name="type_shoes" id="" value="<?php echo $rows['type_shoes']?>">
                      ...
                      <div>
                          <label for="quantity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your Username</label>
                          <input type="quantity" name="quantity" id="quantity" value="<?php echo $rows['quantity']?>" placeholder="1 2 atau lebih" class="bg-gray-50 mb-5 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                      </div>
                      <div>
                          <label for="payment" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your Username</label>
                          <select name="payment">
                          <option value="DANA" <?php if ($rows["payment"] == "DANA") echo "selected"; ?>>DANA</option>
                          <option value="BCA" <?php if ($rows["payment"] == "BCA") echo "selected"; ?>>BCA</option>
                          </select>
                      </div>
